...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 style="color: #333; font-weight: 700;">Edit Stream</h2>
                ...
                <div class="card-body" style="padding: 40px;">
                    <form action="{{ route('admin.streams.update', $stream->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $stream->title) }}" required style="border-radius: 8px; padding: 12px 15px;">
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" style="border-radius: 8px; padding: 12px 15px;">{{ old('description', $stream->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="stream_url" class="form-control @error('stream_url') is-invalid @enderror" value="{{ old('stream_url', $stream->stream_url) }}" required style="border-radius: 8px; padding: 12px 15px;">
                            @error('stream_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="format" class="form-control @error('format') is-invalid @enderror" required style="border-radius: 8px; padding: 12px 15px;">
                                <option value="url" {{ old('format', $stream->format) === 'url' ? 'selected' : '' }}>Regular URL (YouTube, Vimeo)</option>
                                <option value="hls" {{ old('format', $stream->format) === 'hls' ? 'selected' : '' }}>HLS Stream (.m3u8)</option>
                                <option value="rtmp" {{ old('format', $stream->format) === 'rtmp' ? 'selected' : '' }}>RTMP Stream</option>
                            </select>
                            @error('format')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="type" class="form-control @error('type') is-invalid @enderror" required style="border-radius: 8px; padding: 12px 15px;">
                                    <option value="live" {{ old('type', $stream->type) === 'live' ? 'selected' : '' }}>Live Stream</option>
                                    <option value="recorded" {{ old('type', $stream->type) === 'recorded' ? 'selected' : '' }}>Recorded</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="datetime-local" name="scheduled_at" class="form-control @error('scheduled_at') is-invalid @enderror" value="{{ old('scheduled_at', $stream->scheduled_at?->format('Y-m-d\TH:i')) }}" style="border-radius: 8px; padding: 12px 15px;">
                                @error('scheduled_at')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-4">
                            <div class="form-check">
                                <input type="checkbox" name="is_active" class="form-check-input" id="is_active" value="1" {{ old('is_active', $stream->is_active) ? 'checked' : '' }}>
