                ...
                <div class="sidebar-card">
                    <div class="text-center mb-4">
                        <div class="profile-avatar">
                            @if($user->image)
                                <img id="profileImage" src="{{ $user->getImageUrl() }}" alt="Profile Picture" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">
                            @else
                                {{ strtoupper(substr($user->name, 0, 2)) }}
                            @endif
                        </div>
                        <div class="mt-3">
                            <form id="profilePicForm" method="POST" action="{{ route('account.updateProfilePic') }}" enctype="multipart/form-data">
                                @csrf
                        ...
                        <h4 style="margin-bottom: 0.5rem; color: #333;">{{ $user->name }}</h4>
                        <p style="color: #6c757d;">{{ $user->designation ?? 'Member' }}</p>
                    </div>
                    ...
                    <nav>
                        ...
                        <a href="{{ route('account.myGroups') }}" class="nav-link-modern">
            ...
            <div class="col-lg-8">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="text" name="name" id="name" value="{{ $user->name }}" class="form-control" placeholder="Your Name" required>
                                        <label for="name">Full Name</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="email" name="email" id="email" value="{{ $user->email }}" class="form-control" placeholder="Your Email" required>
                                        <label for="email">Email Address</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="text" name="designation" id="designation" value="{{ $user->designation }}" class="form-control" placeholder="Your Designation">
                                        <label for="designation">Designation</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="tel" name="mobile" id="mobile" value="{{ $user->mobile }}" class="form-control" placeholder="Your Mobile">
                                        <label for="mobile">Mobile Number</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="date" name="birthday" id="birthday" value="{{ $user->birthday ? $user->birthday->format('Y-m-d') : '' }}" class="form-control" placeholder="Your Birthday">
                ...
                <div id="password-section" class="section-content" style="display: none;">
                    <div class="modern-card">
                        ...
                        <form id="changePasswordForm" method="POST" action="{{ route('account.updatePassword') }}">
                            @csrf
                            <div class="form-floating">
                ...
                <div id="settings-section" class="section-content" style="display: none;">
                    <div class="modern-card">
                        ...
                        <form id="settingsForm" method="POST" action="{{ route('account.updateSettings') }}">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-6">
                                    <div style="padding: 20px; background: #f8f9fa; border-radius: 12px; margin-bottom: 20px;">
                                        ...
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="email_prayer" id="emailPrayer" {{ $user->email_prayer ? 'checked' : '' }}>
                                            <label class="form-check-label" for="emailPrayer">Prayer updates</label>
                                        </div>
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="email_newsletter" id="emailNews" {{ $user->email_newsletter ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div style="padding: 20px; background: #f8f9fa; border-radius: 12px; margin-bottom: 20px;">
                                        ...
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="public_profile" id="profilePublic" {{ $user->public_profile ? 'checked' : '' }}>
                                            <label class="form-check-label" for="profilePublic">Public profile</label>
                                        </div>
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="show_email" id="showEmail" {{ $user->show_email ? 'checked' : '' }}>
                    ...
                    <div class="modern-card">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            @if($user->unreadNotifications->count() > 0)
                                <a href="{{ route('notifications.markAllAsRead') }}" class="btn btn-sm btn-outline-primary">Mark all as read</a>
                            @endif
                        </div>
                        ...
                        @if($notifications->isNotEmpty())
                            <ul class="list-group list-group-flush">
                                @foreach($notifications as $notification)
                                    <li class="list-group-item d-flex justify-content-between align-items-start py-3 px-0 {{ $notification->read_at ? '' : 'bg-light rounded px-3' }}">
                                        <div class="flex-grow-1">
                                            <a href="{{ route('notifications.markAsRead', ['id' => $notification->id, 'url' => $notification->data['url'] ?? url()->current()]) }}" class="text-decoration-none">
                                                <p class="mb-1 {{ $notification->read_at ? 'text-muted' : 'fw-bold text-dark' }}">
                            ...
                            <div class="col-md-6">
                                <div style="padding: 20px; background: #f8f9fa; border-radius: 12px; margin-bottom: 20px;">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="referralLink" value="{{ url('/account/register?ref=' . $user->referral_code) }}" readonly>
                            ...
                            <div class="col-md-6">
                                <div style="padding: 20px; background: #f8f9fa; border-radius: 12px; margin-bottom: 20px;">
                                    ...
                                    <div class="row text-center">
                                        <div class="col-6">
                                            <div style="font-size: 2rem; font-weight: bold; color: #667eea;">{{ $user->referrals->count() }}</div>
                                            <div style="color: #6c757d;">Total Referrals</div>
                                        </div>
                                        <div class="col-6">
                                            <div style="font-size: 2rem; font-weight: bold; color: #28a745;">{{ $user->referrals->where('email_verified_at', '!=', null)->count() }}</div>
...
$("#settingsForm").submit(function(e){
    ...
    $.ajax({
        url: '{{ route('account.updateSettings') }}',
        type: 'PUT',
        dataType: 'json',
