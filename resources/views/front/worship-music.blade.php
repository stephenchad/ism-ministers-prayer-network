    ...
    <div class="container">
        ...
        <div class="mb-5">
            ...
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                @forelse($streamingMusic as $index => $music)
                <div class="col">
                    <div class="music-card h-100">
                        <div class="p-4">
                            <div class="d-flex align-items-center mb-3">
                                <button class="play-button me-3" onclick="playMusic('music-{{ $music->id }}')">
                                    <i class="fas fa-play" id="play-icon-{{ $music->id }}"></i>
                                    <i class="fas fa-pause" id="pause-icon-{{ $music->id }}" style="display: none;"></i>
                                </button>
                                <div>
                                    <h5 class="fw-bold mb-1">{{ $music->title }}</h5>
                                    <small class="text-muted">{{ $music->artist }} @if($music->duration)• {{ $music->duration }}@endif</small>
                                </div>
                            </div>
                            @if($music->description)
                            <p class="text-muted mb-3">{{ $music->description }}</p>
                            @endif
        ...
        <div class="mb-5">
            ...
            <div class="row row-cols-1 row-cols-md-2 g-4">
                @forelse($downloadMusic as $music)
                <div class="col">
                    <div class="music-card h-100">
                        <div class="p-4">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <div>
                                    <h5 class="fw-bold mb-1">{{ $music->title }}</h5>
                                    <small class="text-muted">{{ $music->artist }} @if($music->file_size)• {{ $music->file_size }}@endif</small>
                                </div>
                                <a href="{{ asset($music->file_path) }}" download class="btn btn-primary btn-sm">
