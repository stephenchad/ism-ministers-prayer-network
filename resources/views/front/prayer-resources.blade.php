...
<div class="py-5 bg-light" style="min-height: 60vh;">
    <div class="container">
                    ...
                    <div class="resource-card">
                        <div class="p-4">
                            <div class="resource-icon">
                                <i class="fas {{ $guide->icon ?: 'fa-book' }}"></i>
                            </div>
                            <h5 class="fw-bold mb-3">{{ $guide->title }}</h5>
                            <p class="text-muted mb-3">{{ $guide->description }}</p>
                            <div class="mb-3">
                                <small class="text-primary fw-semibold">📖 Reading @if($guide->reading_time) • {{ $guide->reading_time }} @endif</small>
                            </div>
                            <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#guideModal{{ $guide->id }}">Read Guide</button>
                        </div>
                    </div>
                </div>
                @endforeach
                ...
                <div class="col-lg-6">
                    <div class="resource-card">
                        <div class="p-4">
                            ...
                            <div class="mb-3">
                                <small class="text-primary fw-semibold">🎥 Video @if($video->duration) • {{ $video->duration }} @endif</small>
                            </div>
                            @if($video->video_type === 'file')
                                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#videoModal{{ $video->id }}">
                                    <i class="fas fa-play me-1"></i>Watch Video
                                </button>
                            @else
                                <a href="{{ $video->video_url }}" target="_blank" class="btn btn-primary btn-sm">
                                    <i class="fas fa-play me-1"></i>Watch Video
                                </a>
                            @endif
                        </div>
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="resource-card">
                        <div class="p-4">
                            ...
                            <h5 class="fw-bold mb-3">{{ $resource->title }}</h5>
                            <p class="text-muted mb-3">{{ $resource->description }}</p>
                            <div class="mb-3">
                                <small class="text-primary fw-semibold">📄 {{ strtoupper($resource->file_type) }} • {{ $resource->file_size }}</small>
                            </div>
                            <a href="{{ route('prayer.download', $resource->id) }}" class="btn btn-primary btn-sm">
                                <i class="fas fa-download me-1"></i>Download {{ strtoupper($resource->file_type) }}
                            </a>
                        </div>
                    </div>
                </div>
        ...
        @if($guides->isEmpty() && $videos->isEmpty() && $resources->isEmpty())
        <div class="text-center py-5">
            ...
            <a href="{{ route('prayers') }}" class="btn btn-primary">
